...
import Editor from "../../Components/editor/Editor"; // Corrected path
import useEditorStore from "../../utils/editorStore";
import apiRequest from "../../utils/apiRequest";
import { useMutation, useQuery } from "@tanstack/react-query";
import BoardForm from "./BoardForm";
import getCroppedImg from "../../utils/cropImg.js"; // <-- 1. IMPORTED CROP HELPER
...
const CreatePage = () => {
  ...
  const handleSubmit = async () => {
    if (isEditing) {
      // "Done" button logic
      try {
        const { croppedAreaPixels } = useEditorStore.getState();
        const croppedImage = await getCroppedImg(
          previewImg.url,
          croppedAreaPixels
        );

        // Update the main file and preview state
      ...
      // REMOVED textOptions and canvasOptions
      ...
      {isEditing ? (
        ...
        <div className="createBottom">
          ...
          <form className="createForm" ref={formRef}>
            {/* ... all your form items (title, description, etc) are unchanged ... */}
